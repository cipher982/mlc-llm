...
def test_block_realize_predicate_conversion(predicate_input, expected_value):
    ...
    # This should NOT raise an error - the fix handles all these cases
...
def test_block_realize_with_nested_blocks():
    """Test nested BlockRealize structures with bool predicates."""
    ...
    inner_realize = tir.BlockRealize(
        iter_values=[],
        predicate=tir.IntImm("bool", 1),  # Legacy bool dtype
        block=inner_block,
    )

    outer_block = tir.Block(
        iter_vars=[],
        reads=[],
        writes=[],
        name_hint="outer",
    ...
    outer_realize = tir.BlockRealize(
        iter_values=[],
        predicate=True,  # Python bool
        block=outer_block,
    )

    # Both predicates should be uint1
    assert outer_realize.predicate.dtype == "uint1"
    assert outer_realize.block.body.predicate.dtype == "uint1"
